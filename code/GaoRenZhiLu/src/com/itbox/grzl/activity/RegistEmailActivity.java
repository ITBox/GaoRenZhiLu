...
public class RegistEmailActivity extends BaseActivity {
	...
	public void registEmailAccount() {
		...
		Net.request("useremail", email, Api.getUrl(Api.User.CheckAccount),
				new GsonResponseHandler<CheckAccount>(CheckAccount.class) {
					@Override
					public void onSuccess(CheckAccount object) {
						...
						if (userEmail == 0) {
							...
							ToastUtils.makeCustomPosition(mActThis, "该邮箱已被注册",
	...
	private void registEmailAll() {
		final String email = mLLEmailName.getText().toString();
		final String pass = mLLEmailPass.getText().toString();
				...
				new GsonResponseHandler<Register>(Register.class) {
					...
					public void onSuccess(Register object) {
						...
						if (object.getResult() > 0) {
							...
							new LoginAndRegisterApi().login(email, pass,
	...
	@OnClick({ R.id.regist_email_sex, R.id.regist_email_birthday,
			R.id.regist_email_area, R.id.regist_email_type, R.id.regist_phone,
			R.id.regist_email_clause })
